from dataset_preparation import build_corpus, build_corpus_test, build_map_tags, build_map_words, sentence_vectorizer,tag_vectorizer
from model1 import BiLSTM
...
dataloader_training = DataLoader(dataset=training_data,
                        batch_size=8,
                        drop_last=True,
...
for i in range(1, epochs+1):
    train_loss = 0.0
    for training_data, training_tag, data_len, tag_len in dataloader_training:
        optimizer.zero_grad()
        output = BiLSTM_model(training_data, data_len)
        output = output.view(-1, len(tag2id))
        training_tag = training_tag.view(-1)
        loss = criterion(output, training_tag)
        # print(loss)
        loss.backward()
        optimizer.step()
        train_loss += loss.item() * training_data.size(1)
    train_loss = train_loss / len(dataloader_training.dataset)
    ...
    for i in range(len(dev_data)):
        for j in range(len(dev_data[i])):
            if dev_data[i][j] != 0:
                ...
                tagp = id2tag[pred[i][j]]
                dev_pred.append((word, gold, tagp))

print('creating predictions file for validationg and calculating F1 score')
...
for test_data,test_data_len in dataloader_test:
    pred = BiLSTM_model(test_data, test_data_len)
    pred = pred.cpu()
    pred = pred.detach().numpy()
    ...
    for i in range(len(test_data)):
        for j in range(len(test_data[i])):
            if test_data[i][j] != 0:
                word = id2word[test_data[i][j]]
                tag = id2tag[pred[i][j]]
                test_pred.append((word, tag))
print('creating test1.out')
